...
class Volume extends Command {
  ...
  async run(message, args, data) {
    ...
    if (!args[0] || isNaN(args[0]) || args[0] === "Infinity") return message.error("Please enter a valid number!");
    if (Math.round(parseInt(args[0])) < 1 || Math.round(parseInt(args[0])) > 100) return message.error("Please enter a valid number (between 1 and 100)!");

    const success = this.client.player.setVolume(message, parseInt(args[0]));
    if (success) message.success(`Volume set to **${parseInt(args[0])}%**!`);
  }
}
