  ...
  async run(message, args, data) {
    ...
    if (args.join(" ").toLowerCase() === "queue") {
      if (this.client.player.getQueue(message).loopMode) {
        ...
        return message.success("Repeat mode **disabled**!");
      } else {
        this.client.player.setLoopMode(message, true);
        return message.success("Repeat mode **enabled** the whole queue will be repeated endlessly!");
      }
    } else {
      if (this.client.player.getQueue(message).repeatMode) {
        ...
        return message.success("Repeat mode **enabled** the current song will be repeated endlessly!");
      }
    }
  }
}
