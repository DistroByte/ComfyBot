...
class Setwarns extends Command {
  ...
  async run(message, args, data) {
    ...
    if (!sanction || (sanction !== "kick" && sanction !== "ban")) {
      return message.error("Please specify sanction between `kick` and `ban`");
    }
    const number = args[1];
    ...
    if (number === "reset") {
      if (sanction === "kick") {
        ...
        data.guild.save();
        return message.success(`**Members can no longer be automatically kicked!**\n\n:arrow_right_hook: *Send \`${data.guild.prefix}configuration\` to see the updated configuration!*`);
      }
      if (sanction === "ban") {
        data.guild.plugins.warnsSanctions.ban = false;
        data.guild.markModified("plugins.warnsSanctions");
        data.guild.save();
        return message.success(`**Members can no longer be automatically banned!**\n\n:arrow_right_hook: *Send \`${data.guild.prefix}configuration\` to see the updated configuration!*`);
      }
    }
    ...
    if (sanction === "kick") {
      ...
      data.guild.markModified("plugins.warnsSanctions");
      data.guild.save();
      return message.success(`**\`${number}\` warnings will result in a kick!**\n\n:arrow_right_hook: *Send \`${data.guild.prefix}configuration\` to see the updated configuration!*`);
    }
