    ...
    function prettyPrint(fetchedChan, limit = 10) {
      fetchedChan.messages.fetch({ limit }).then(fetched => {
        ...
        fetched.forEach(msg => {
          ...
          msgString = `\`${new Date(msg.createdTimestamp).toUTCString()}\` # ${msg.member.displayName ? msg.member.displayName : msg.member.username}: ${msg.content.replace(/((https?:\/\/)?[^\s.]+\.[\w][^\s]+)/gm, "<$&>")}`;
