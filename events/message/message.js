...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.guild) {
      ...
      if (message.guild.id === "713522800081764392") {
        ...
        if (message.content.toLowerCase().trim().split(/ +/g).includes("u")) {
          ...
          insultCooldown = Date.now() + 900000;
    ...
    client.logger.log(`| ${message.author.username} used ${cmd.help.name} in ${(message.guild ? message.guild.name : "DMs")} | ${cmd.help.name} ${args.join(" ")}`, "cmd");
